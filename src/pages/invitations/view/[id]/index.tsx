            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Invitation Invited By Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_invitation_invited_byTouser?.id}`}>
                        {data?.user_invitation_invited_byTouser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Invitation Invitee Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_invitation_inviteeTouser?.id}`}>
                        {data?.user_invitation_inviteeTouser?.email}
                      </Link>
                    }
                  />
                )}
